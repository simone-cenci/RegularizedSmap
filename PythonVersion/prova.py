...
#%%
ts = np.loadtxt('deterministic_chaos_cr.txt')
jac = np.loadtxt('jacobian_chaos_cr.txt')
cross_validation_options = ['LOOCV', 'RollingCV']
specie_options = ['all', 'each']
sp = specie_options[0]
...
#    e,l,t = cv.loocv(parameters, training_set, par = True)
#    print(' ... done')
#else:
    ### Or Rolling window cross validation:
#    print('Running:', cross_validation_type, '... This will take a while ...')
#    e,l,t = cv.rollingcv(parameters, unscaled_training_set, 20, par = True)
#    print(' ... done')


#%%
#lm = np.array([parameters[k]['lambda'] for k in range(len(e))])
#th = np.array([parameters[k]['theta'] for k in range(len(e))])
#a = np.column_stack([lm,th,e])
#np.savetxt('error_landscape.txt',  a)


#%%
#### In sample statistics
smap_object = smr.SMRidge(0.0001,5)
...
stat.pearsonr(np.delete(true_vcr, 0), infered_vcr)[0])
